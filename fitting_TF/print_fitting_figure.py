    ...
    for i in range(result_n.shape[2]):
        ...
        for j in range(nb_value_finh):
            for k in range(nb_value_fexc):
                ...
                point = dict(markerfacecolor=color_line,marker='.',linestyle='none',markeredgewidth=0.1,markersize=5.0)
                plt.boxplot(result_n[k,j,i,6:]*1e3, positions=[result_n[k,j,i,1]*1e3], notch=True,widths=0.00001,flierprops=point,
                            showmeans=True,showbox=False,whiskerprops=whiskerprops,meanprops=meanpointprops,medianprops=medianprops
                            )
...
def print_result_curve_box_std(result_n,name_fig,nb_value_finh,nb_value_fexc):
    ...
    for i in range(result_n.shape[2]):
        for j in range(nb_value_finh):
            ...
            # print mean + std
            plt.plot(result_n[:, j, i, 1] * 1e3, result_n[:, j, i, 4] * 1e3 + result_n[:, j, i, 0] * 1e3, 'r',alpha=0.5)
            # print mean - std
            plt.plot(result_n[:, j, i, 1] * 1e3, -result_n[:, j, i, 4] * 1e3 + result_n[:, j, i, 0] * 1e3, 'b',alpha=0.5)
            plt.ylim(ymax=np.max(result_n[:,j,i,6:]*1e3+0.5))
            plt.title('adapt = '+str(result_n[:,j,i,3][0]) + ' inhibition :'+str(result_n[:,j,i,2][0] *1e3),{"fontsize":30.0})
            plt.ylabel('output frequency of the neurons in Hz',{"fontsize":30.0})
            plt.xlabel('excitatory input frequency in Hz',{"fontsize":30.0})
            ...
            for k in range(nb_value_fexc):
                if not np.isnan(result_n[k,j,i,0]):
                    plt.plot([result_n[k,j,i,1]*1e3,result_n[k,j,i,1]*1e3],
                             [result_n[k,j,i,0]*1e3,function_fit(P_relatif,result_n[k,j,i,1],result_n[k,j,i,2], result_n[k,j,i,3])*1e3],color='r',alpha=0.5,linewidth=0.5,linestyle='dashed',)
                    plt.plot([result_n[k,j,i,1]*1e3,result_n[k,j,i,1]*1e3],
                             [result_n[k,j,i,0]*1e3,function_fit(P_absolute,result_n[k,j,i,1],result_n[k,j,i,2], result_n[k,j,i,3])*1e3],color='k',alpha=0.5,linewidth=0.5,linestyle='dashed',)
        plt.ylim(ymax=50.0)
        name_fig_i = name_fig + str(i) + '.svg'
        plt.savefig(name_fig_i)
        plt.close('all')
    ...
    for i in range(result_n.shape[2]):
        ...
        for j in range(nb_value_finh):
            for k in range(nb_value_fexc):
                if not np.isnan(result_n[k,j,i,0]):
                    plt.plot([result_n[k,j,i,1]*1e3,result_n[k,j,i,1]*1e3],
                             [result_n[k,j,i,0]*1e3,function_fit(P,result_n[k,j,i,1],result_n[k,j,i,2], result_n[k,j,i,3])*1e3],color='r',alpha=0.5,linewidth=0.5)
        plt.ylim(ymax=50.0)
        name_fig_i = name_fig + str(i) + '.svg'
        plt.savefig(name_fig_i)
        plt.close('all')
    ...
    for i in range(result_n.shape[2]):
        ...
        for j in range(nb_value_finh):
            for k in range(nb_value_fexc):
                if not np.isnan(result_n[k,j,i,0]):
                    # plt.plot([result_n[k,j,i,1]*1e3,result_n[k,j,i,1]*1e3],
                    # [result_n[k,j,i,0]*1e3,TF(result_n[k,j,i,1],result_n[k,j,i,2] , p_without,w=result_n[k,j,i,3])*1e3],color='r',alpha=0.5)
                    # print error with adaptation
                    plt.plot([result_n[k, j, i, 1]*1e3, result_n[k, j, i, 1]*1e3], [result_n[k, j, i, 0]*1e3, TF(result_n[k, j, i, 1], result_n[k, j, i, 2], p_with, w=result_n[k, j, i, 3])*1e3], color='r', alpha=0.5,linewidth=0.5)
        name_fig_i = name_fig + str(i) + '.svg'
        plt.ylim(ymax=50.0)
        plt.savefig(name_fig_i)
        plt.close('all')
