...
file = PdfPages('/home/kusch/Documents/project/Zerlaut/compare_zerlaut/parameter_analyse/compare_rate_'+str(rate)+'.pdf')
for frequency in frequencies:
    ...
    for amplitude_index, amplitude in enumerate(amplitudes):
        for begin, end in [(0.0, 20000.0-0.1), (0.0, 1000.0), (19000.0, 20000.0-0.1)]:
            ...
            path_simulation_network = path_result_network + '/rate_' + str(rate) + '/_frequency_'+ str(frequency) + '_amplitude_' + str(np.around(amplitude, 2)) +'/'
            ...
            histogram_0_1s_ex = np.array(np.histogram(spikes_concat_ex, bins=int((end - begin) / resolution)), dtype=object)
            ...
            histogram_0_1s_in = np.array(np.histogram(spikes_concat_in, bins=int((end - begin) / resolution)), dtype=object)
            ...
            f_ex, Pxx_den_ex = signal.welch(histogram_5w_ex, fs=1 / resolution * 1e3, nperseg=int((end - begin) / resolution)-int(window_size/resolution))
            ax.semilogy(f_ex, Pxx_den_ex)
            f_in, Pxx_den_in = signal.welch(histogram_5w_in, fs=1 / resolution * 1e3, nperseg=int((end - begin) / resolution)-int(window_size/resolution))
            ax.semilogy(f_in, Pxx_den_in)
            ax.set_title('average mean of the histogram with resolution of '+str(resolution)+' ms and a window of 5ms')
            ...
            f_ex, Pxx_den_ex = signal.welch(histogram_0_1s_ex[0], fs=1 / resolution * 1e3, nperseg=int((end - begin) / resolution)-1)
            ax.semilogy(f_ex, Pxx_den_ex)
            f_in, Pxx_den_in = signal.welch(histogram_0_1s_in[0], fs=1 / resolution * 1e3, nperseg=int((end - begin) / resolution)-1)
            ax.semilogy(f_in, Pxx_den_in)
