...
Cnt=0
Lseed= [i for i in range(50)]  #[i*2 for i in range(60)]
...
for Nseed in Lseed:
	for ExIn in Lei:
		...
		#Create populations:
			...
			# Population 1 - FS
		bVal=30
		b1 = bVal*pA #no adaptation for FS
		...
		np.save('Results3/AD_popRateInh_ExIn_'+str(ExIn)+'_bval_'+str(bVal)+'Nseed_'+str(Nseed)+'.npy', popRateG1)
		np.save('Results3/AD_popRateExc_ExIn_'+str(ExIn)+'_bval_'+str(bVal)+'Nseed_'+str(Nseed)+'.npy', popRateG2)
