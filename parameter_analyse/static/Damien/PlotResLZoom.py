...
for bVal in Lb:
	...
	for ExIn in Lei:
		...
		FRinh = np.load('Results/AD_popRateInh_ExIn_'+str(ExIn)+'_bval_'+str(bVal)+'Nseed_'+str(0)+'.npy')
		FRexc = np.load('Results/AD_popRateExc_ExIn_'+str(ExIn)+'_bval_'+str(bVal)+'Nseed_'+str(0)+'.npy')
...
plt.figure()
plt.imshow(hmRes,origin='lower', extent=[0,2,0,120], aspect='auto')
