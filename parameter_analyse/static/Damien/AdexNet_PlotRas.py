...
Cnt=0
Lb=[85]  #[i*2 for i in range(60)]
Lei=[80] #[i*0.1 for i in range(40)]
for bVal in Lb:
	for ExIn in Lei:
		Cnt+=1
		#########################################################
		...
		#np.save('Results/AD_popRateInh_ExIn_'+str(ExIn)+'_bval_'+str(bVal)+'Nseed_'+str(0)+'.npy', popRateG1)
		#np.save('Results/AD_popRateExc_ExIn_'+str(ExIn)+'_bval_'+str(bVal)+'Nseed_'+str(0)+'.npy', popRateG2)
...
RasG1 = array([M1G1.t/ms, [i+N2 for i in M1G1.i]])
RasG2 = array([M1G2.t/ms, M1G2.i])
