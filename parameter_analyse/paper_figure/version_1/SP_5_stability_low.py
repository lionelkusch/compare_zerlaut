...
firing_rate_30 = np.concatenate((np.array([np.arange(48.0, 99.0, 1.0)]).swapaxes(0, 1), firing_rate_30), axis=1)
firing_rate_60 = np.load(path + '/b_60.0/firing_rate.npy')
firing_rate_60 = np.concatenate((np.array([np.arange(48.0, 99.0, 1.0)]).swapaxes(0, 1), firing_rate_60), axis=1)
...
for index, (network, firing_rate, b, color, title) in enumerate([
                                                                 (network_0,  firing_rate_0,  b_0,  'k', 'b=0 pA'),
    ...
    plt.xlim(xmin=48.0, xmax=100.0)
    plt.xticks([48.0, 75.0, 100.0])
