...
linewidth_mean_field = 2.0
linewidth_stability = 1.0
marker_size = 10.0
...
ax = plt.subplot(236)
name_spike_pop = ['spike of\nexcitatory neurons', 'spike of\ninhibitory neurons']
for pop, [neurons_id, times_spike] in enumerate(data_pop_all_80.values()):
    ax.scatter(times_spike, neurons_id, c=color[pop], s=spike_size, label=name_spike_pop[pop])
points = ax.get_legend_handles_labels()
ax.set_xlim(xmax=end + 10.0, xmin=begin - 10.0)
ax.tick_params(axis='both', labelsize=ticks_size)
ax.set_xlabel('time (ms)', {"fontsize": labelticks_size}, labelpad=2.5)

ax = plt.subplot(233)
ax.plot(times_80[:-int(window / dt)], hist_slide_ex_80, linewidth=linewidth_network, c='r', label='network:\nexcitatory population')
ax.plot(times_80[:-int(window / dt)], hist_slide_in_80, linewidth=linewidth_network, c='b', label='network:\ninhibitory population')
ax.plot(times_80, rateE_80, linewidth=linewidth_mean_field, c='orange', label='mean field:\nexcitatory population')
ax.plot(times_80, rateI_80, linewidth=linewidth_mean_field, c='cyan', label='mean field:\ninhibitory population')
ax.tick_params(axis='both', labelsize=ticks_size)
ax.set_xticks([])

handles, labels = ax.get_legend_handles_labels()
handles = handles + points[0]
labels = labels + points[1]
fig.legend(handles, labels, loc='center right', fontsize=labelticks_size)
plt.subplots_adjust(top=0.985, bottom=0.105, left=0.065, right=0.77, wspace=0.26, hspace=0.045)

# plt.show()
plt.savefig('./figure/figure_0_presentation.png', dpi=300)
