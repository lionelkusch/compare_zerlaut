...
index_b_0 = np.argmin(np.abs(b_0['x'][6]-rate))

b_30 = loadmat(path + '/b_30/EQ_Low/EQ_Low.mat', chars_as_strings=True, simplify_cells=True)
b_30['x'][:2] *= 1e3
b_30['x'][-1] *= 1e3
index_b_30 = np.argmin(np.abs(b_30['x'][6]-rate))

b_60 = loadmat(path + '/b_60/EQ_Low/EQ_Low.mat', chars_as_strings=True, simplify_cells=True)
b_60['x'][:2] *= 1e3
b_60['x'][-1] *= 1e3
index_b_60 = np.argmin(np.abs(b_60['x'][6]-rate))


for index, (d_b_0, d_b_30, d_b_60) in enumerate([(rateE_b_0, rateE_b_30, rateE_b_60),
    ...
    plt.hlines(b_0['x'][index][index_b_0], colors='blue', linestyles='dashed', xmin=0.0, xmax=2000.0)
    plt.plot(times_b_30, d_b_30, linewidth=linewidth_mean_field, c='red')
    plt.hlines(b_30['x'][index][index_b_30], colors='red', linestyles='dashed', xmin=0.0, xmax=2000.0)
    plt.plot(times_b_60, d_b_60, linewidth=linewidth_mean_field, c='orange')
    plt.hlines(b_60['x'][index][index_b_60], colors='orange', linestyles='dashed', xmin=0.0, xmax=2000.0)
