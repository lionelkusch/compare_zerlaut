...
def draw_contour(fig, ax, X, Y, Z, levels, title, xlabel, ylabel, zlabel, zmin, zmax, label_size,
                 number_size, cmap=None, size=6.0):
    """
    ...
    :return:
    """
    CS = ax.tricontourf(X, Y, Z, False, levels=levels, vmin=zmin, vmax=zmax, extend='both', cmap=cmap)
    # ax.plot(X, Y, 'w.', markersize=size)
...
name_figure = './figure_compare_T_rate_'+str(rate)+'_pop_'+population+'.png'

# chose T value
Ts = [0.2, 0.5, 1, 5, 50, 150]
levels_max = [10.0, 20.0, 30.0, 35.0, 40.0, 45.0, 50.0, 60.0] if population == 'excitatory' else [20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0, 60.0]
levels = [0., 5.0, 10.0, 15.0, 20.0] if population == 'excitatory' else [20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0]

## make figure
fig = plt.figure(figsize=(20., 20.))

#### network
ax = plt.subplot(1+len(Ts), 4, 1)
data_network = getData(data_base_network, table_name_network, list_variable, population)
X, Y, Z = grid(data_network['amplitude'], data_network['frequency'], data_network['PLV_0_1ms'], res=False,
               resX=None, resY=None)
draw_contour(fig, ax, X, Y, Z, [0., 0.5, 0.6, 0.7, 0.8, 0.9, 1.], '', '', '', '', 0.0, 1.0, 10.0, ticks_size)
# draw_point(ax, X, Y, size=2.0)
ax.set_ylabel('Spiking Neural Network\nfrequency of input (Hz)', fontdict={'fontsize': labelticks_size})
ax.set_title('Phase Locking Value', fontdict={'fontsize': labelticks_size})
ax.tick_params(labelsize=ticks_size)

ax = plt.subplot(1+len(Ts), 4, 2)
data_network = getData(data_base_network, table_name_network, list_variable, population)
X, Y, Z = grid(data_network['amplitude'], data_network['frequency'], data_network['MeanPhaseShift_0_1ms'], res=False,
               resX=None, resY=None)
draw_contour(fig, ax, X, Y, Z, np.linspace(-1, 1., 10), #[-1., -0.75, -0.5, -0.25, 0., 0.25, 0.5, 0.75, 1.],
             '', '', '', '', -1, 1., 10.0,
             ticks_size)
# draw_point(ax, X, Y, size=2.0)
...
ax = plt.subplot(1+len(Ts), 4, 3)
data_network = getData(data_base_network, table_name_network, list_variable, population)
X, Y, Z = grid(data_network['amplitude'], data_network['frequency'], data_network['max_IFR_0_1ms'], res=False,
               resX=None, resY=None)
draw_contour(fig, ax, X, Y, Z, levels_max, '', '', '', '', levels_max[0],  levels_max[-1], 10.0, ticks_size)
# draw_point(ax, X, Y, size=2.0)
ax.set_title('Maximum firing rate (Hz)', fontdict={'fontsize': labelticks_size})
ax.tick_params(labelsize=ticks_size)
ax.set_yticks([])

ax = plt.subplot(1+len(Ts), 4, 4)
data_network = getData(data_base_network, table_name_network, list_variable, population)
X, Y, Z = grid(data_network['amplitude'], data_network['frequency'], data_network['rates_average'], res=False,
               resX=None, resY=None)
draw_contour(fig, ax, X, Y, Z, levels, '', '', '', '', levels[0],  levels[-1], 10.0, ticks_size)
# draw_point(ax, X, Y, size=2.0)
ax.set_title('Mean firing rate (Hz)', fontdict={'fontsize': labelticks_size})
ax.tick_params(labelsize=ticks_size)
ax.set_yticks([])

for index, T in enumerate(Ts):
    data_base_mean = path_mean + '/deterministe/T_'+str(T)+'/database.db'
    #### mean field
    ax = plt.subplot(1+len(Ts), 4, (index+1)*4+1)
    ...
    draw_contour(fig, ax, X, Y, Z, [0., 0.5, 0.6, 0.7, 0.8, 0.9, 1.], '', '', '', '', 0.0, 1.0, 10.0, ticks_size)
    # draw_point(ax, X, Y, size=2.0)
    # ax.set_ylabel('Mean Field Approximation\nfrequency of input (Hz)', fontdict={'fontsize': labelticks_size})
    ax.set_ylabel('T='+str(T), fontdict={'fontsize': labelticks_size})
    ...
    #### mean field
    ax = plt.subplot(1+len(Ts), 4, (index+1)*4+2)
    data_network = getData(data_base_mean, table_name_mean, list_variable, population, cond=" AND rate == " + str(rate))
    ...
    #### mean field
    ax = plt.subplot(1+len(Ts), 4, (index+1)*4+3)
    data_network = getData(data_base_mean, table_name_mean, list_variable, population, cond=" AND rate == " + str(rate))
    ...
    #### mean field
    ax = plt.subplot(1+len(Ts), 4, (index+1)*4+4)
    data_network = getData(data_base_mean, table_name_mean, list_variable, population, cond=" AND rate == " + str(rate))
    X, Y, Z = grid(data_network['amplitude'], data_network['frequency'], data_network['mean_rates'], res=False,
                   resX=None, resY=None)
    draw_contour(fig, ax, X, Y, Z, levels, '', '', '', '', levels[0],  levels[-1], 10.0, ticks_size)
    # draw_point(ax, X, Y, size=2.0)
    if index == len(Ts)-1:
...
plt.subplots_adjust(top=0.95, bottom=0.085, left=0.10, right=0.975, hspace=0.12, wspace=0.205)
# plt.show()
